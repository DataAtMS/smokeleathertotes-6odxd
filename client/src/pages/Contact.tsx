...
export default function Contact() {
  ...
  const contactEmail = '';
  const fallbackUrl = 'https://laticoleathers.com';
  const COLOR = {
    primary: "#f0f0f0",
    body: "#c8c8c8",
    secondary: "#999",
  ...
  useEffect(() => {
    document.title = 'Contact | Smoke Leather Totes';
    ...
    const desc = 'Get in touch with the Smoke Leather Totes team. We read every message.';
    setMeta('meta[name="description"]', 'description', desc, 'name');
    setMeta('meta[property="og:title"]', 'og:title', 'Contact | Smoke Leather Totes', 'property');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (contactEmail) {
      const subject = encodeURIComponent('Message from ' + name + ' via Smoke Leather Totes');
          ...
          <div style={{ background: "#0d1a0d", border: "1px solid #1a4a1a", borderRadius: "4px", padding: "24px", textAlign: "center" }}>
            ...
            <p style={{ fontFamily: "Georgia, serif", fontSize: "14px", color: COLOR.secondary }}>
              ...
              {contactEmail ? (
                ...
                <a href={fallbackUrl} target="_blank" rel="noopener noreferrer" style={{ color: COLOR.link }}>Latico Leathers</a>
              )}.
            </p>
          </div>
        ) : (
